/**
 * HSCode 列表页：主键 hsCode，path /logistics/hs-codes，详情 /logistics/hs-codes/{hsCode}。删除若返回 {ok:false,message} 需提示。
 */
...
function LogisticsHsCodeListPage({ onOpenDetail }) {
  ...
  const [form, setForm] = useState({ hsCode: '', nameCn: '', nameEn: '', status: 'enabled' });
  const [showImportModal, setShowImportModal] = useState(false);
  const [importText, setImportText] = useState('');

  const loadList = useCallback(() => {
    setListData(logisticsService.hsCodes.list({ keyword: filters.keyword, status: filters.status, approvalStatus: filters.approvalStatus, page, pageSize: PAGE_SIZE }));
  }, [filters.keyword, filters.status, filters.approvalStatus, page]);

  const loadStats = useCallback(() => {
    const all = logisticsService.hsCodes.list({ page: 1, pageSize: 99999 });
    const enabled = logisticsService.hsCodes.list({ status: 'enabled', page: 1, pageSize: 1 }).total;
    const pending = logisticsService.hsCodes.list({ approvalStatus: 'pending', page: 1, pageSize: 1 }).total;
  ...
  const openDetail = (row) => {
    onOpenDetail && onOpenDetail({
      id: `hscode-${row.hsCode}`,
      name: `HS-${row.hsCode}`,
      path: `/logistics/hs-codes/${encodeURIComponent(row.hsCode)}`,
      data: { id: row.hsCode },
    });
  };

  const openCreate = () => { setEditingRow(null); setForm({ hsCode: '', nameCn: '', nameEn: '', status: 'enabled' }); setShowFormModal(true); };

  const openEdit = (row) => {
    setEditingRow(row);
    setForm({ hsCode: row.hsCode || '', nameCn: row.nameCn || '', nameEn: row.nameEn || '', status: row.status || 'enabled' });
  ...
  const handleExport = () => {
    const res = logisticsService.hsCodes.list({ keyword: filters.keyword, status: filters.status, approvalStatus: filters.approvalStatus, page: 1, pageSize: 99999 });
    const blob = new Blob([JSON.stringify(res.list, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `HSCode_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const handleImport = () => {
    ...
    arr.forEach((item) => {
      ...
      const existing = pk ? logisticsService.hsCodes.get(pk) : null;
      if (existing) { logisticsService.hsCodes.update(pk, { ...item, hsCode: undefined }); updated++; }
      else { logisticsService.hsCodes.create(item); created++; }
    });
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <h1 className="text-lg font-semibold text-gray-800 mb-4">HSCode</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-4 border border-gray-200">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <input type="text" placeholder="编码/中文名/英文名" value={filters.keyword} onChange={(e) => setFilters((f) => ({ ...f, keyword: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm w-48 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={openCreate} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">新增 HSCode</button>
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-gray-50 sticky top-0 z-[1]">
              <tr className="border-b">
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600 whitespace-nowrap">中文名称</th>
            ...
            <tbody>
              {listData.list.map((row) => (
                <tr key={row.hsCode} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-gray-800">{row.hsCode}</td>
                  <td className="px-4 py-3">
                    <button type="button" onClick={() => openDetail(row)} className="text-blue-600 hover:underline text-left font-medium">{row.nameCn || '-'}</button>
                  </td>
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-[440px] p-6">
            <h2 className="text-lg font-semibold mb-4">{editingRow ? '编辑 HSCode' : '新增 HSCode'}</h2>
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-gray-700 mb-1">HS 编码 *</label>
                <input type="text" value={form.hsCode} onChange={(e) => setForm((f) => ({ ...f, hsCode: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" placeholder="如 01012100" readOnly={!!editingRow} />
              </div>
              <div><label className="block text-sm text-gray-700 mb-1">中文名称</label><input type="text" value={form.nameCn} onChange={(e) => setForm((f) => ({ ...f, nameCn: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">英文名称</label><input type="text" value={form.nameEn} onChange={(e) => setForm((f) => ({ ...f, nameEn: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-[560px] p-6 max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <p className="text-sm text-gray-500 mb-3">粘贴 JSON 数组，同 hsCode 更新，否则新增。</p>
...
export default LogisticsHsCodeListPage;
