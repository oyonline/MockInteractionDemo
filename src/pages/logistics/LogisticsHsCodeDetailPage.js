/**
 * HSCode 详情页：主键 hsCode，id 取自 tab.data?.id 或 path 最后一段（decodeURIComponent）。entityType 'hsCode'。
 */
...
function LogisticsHsCodeDetailPage({ tab }) {
  const pathPart = (tab?.path || '').split('/').pop();
  const id = tab?.data?.id ?? (pathPart ? decodeURIComponent(pathPart) : '');
  const [record, setRecord] = useState(null);
  const [form, setForm] = useState({ hsCode: '', nameCn: '', nameEn: '', status: 'enabled', updatedAt: '' });
  ...
  useEffect(() => {
    if (!id) return;
    const r = logisticsService.hsCodes.get(id);
    setRecord(r);
    if (r) {
      setForm({ hsCode: r.hsCode || '', nameCn: r.nameCn || '', nameEn: r.nameEn || '', status: r.status || 'enabled', updatedAt: r.updatedAt || '' });
      setApprovalRecords(logisticsService.approvals.listByEntity('hsCode', id));
    }
  }, [id]);
  ...
  const refresh = () => {
    ...
    setRecord(r);
    if (r) {
      setForm((prev) => ({ ...prev, nameCn: r.nameCn, nameEn: r.nameEn, status: r.status, updatedAt: r.updatedAt || '' }));
      setApprovalRecords(logisticsService.approvals.listByEntity('hsCode', id));
    }
  };
  ...
  const saveForm = () => {
    if (!id) return;
    logisticsService.hsCodes.update(id, { nameCn: form.nameCn, nameEn: form.nameEn, status: form.status });
    window.alert('已保存');
    refresh();
  };

  const handleSubmit = () => {
    const res = logisticsService.hsCodes.submit(id);
    if (res && res.ok === false) window.alert(res.message || '提交失败');
    else refresh();
  };

  const handleWithdraw = () => {
    const res = logisticsService.hsCodes.withdraw(id);
  ...
  const submitRemark = () => {
    if (remarkAction === 'approve') { const res = logisticsService.hsCodes.approve(id, remarkText); if (res && res.ok === false) window.alert(res.message || '操作失败'); }
    else if (remarkAction === 'reject') { const res = logisticsService.hsCodes.reject(id, remarkText); if (res && res.ok === false) window.alert(res.message || '操作失败'); }
    setShowRemarkModal(false);
    refresh();
  };

  if (!id) return <div className="p-6 text-gray-500">缺少 HS 编码</div>;
  if (record === null) return <div className="p-6"><p className="text-gray-500 mb-4">未找到该 HSCode</p><p className="text-sm text-gray-400">请从列表页重新打开或切换其他 Tab。</p></div>;
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-auto p-6">
      <h1 className="text-lg font-semibold text-gray-800 mb-4">HSCode 详情</h1>
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="text-base font-medium text-gray-800 mb-4 border-l-4 border-blue-600 pl-2">基础信息</h2>
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><label className="block text-sm text-gray-600 mb-1">HS 编码</label><input type="text" value={form.hsCode} readOnly className="w-full border border-gray-200 rounded px-3 py-2 text-sm bg-gray-50" /></div>
          <div><label className="block text-sm text-gray-600 mb-1">中文名称</label><input type="text" value={form.nameCn} onChange={(e) => setForm((f) => ({ ...f, nameCn: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
          <div><label className="block text-sm text-gray-600 mb-1">英文名称</label><input type="text" value={form.nameEn} onChange={(e) => setForm((f) => ({ ...f, nameEn: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
...
export default LogisticsHsCodeDetailPage;
