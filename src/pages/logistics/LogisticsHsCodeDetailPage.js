...
function LogisticsHsCodeDetailPage({ tab }) {
  ...
  const submitRemark = () => {
    if (remarkAction === 'approve') { const res = logisticsService.hsCodes.approve(id, remarkText); if (res && res.ok === false) toast.error(res.message || '操作失败'); }
    else if (remarkAction === 'reject') { const res = logisticsService.hsCodes.reject(id, remarkText); if (res && res.ok === false) toast.error(res.message || '操作失败'); }
