/**
 * 集货规则列表页：统计、筛选、表格、ActionBar、新增/编辑/导入/导出、分页。
 */
...
function LogisticsConsolidationRuleListPage({ onOpenDetail }) {
  ...
  const [form, setForm] = useState({ name: '', country: '', weight: 0, status: 'enabled', platform: '', vendorId: '', channelId: '', taxIncluded: '', maxOrdersPerBatch: 50, cutoffHourLocal: '' });
  ...
  const loadList = useCallback(() => {
    setListData(logisticsService.consolidationRules.list({ keyword: filters.keyword, status: filters.status, approvalStatus: filters.approvalStatus, page, pageSize: PAGE_SIZE }));
  }, [filters.keyword, filters.status, filters.approvalStatus, page]);

  const loadStats = useCallback(() => {
    const all = logisticsService.consolidationRules.list({ page: 1, pageSize: 99999 });
    const enabled = logisticsService.consolidationRules.list({ status: 'enabled', page: 1, pageSize: 1 }).total;
    const pending = logisticsService.consolidationRules.list({ approvalStatus: 'pending', page: 1, pageSize: 1 }).total;
  ...
  const openDetail = (row) => {
    onOpenDetail && onOpenDetail({ id: `consol-${row.id}`, name: `集货规则-${row.name}`, path: `/logistics/rules/consolidation/${row.id}`, data: { id: row.id } });
  };

  const openCreate = () => {
    setEditingRow(null);
    setForm({ name: '', country: '', weight: 0, status: 'enabled', platform: '', vendorId: '', channelId: '', taxIncluded: '', maxOrdersPerBatch: 50, cutoffHourLocal: '' });
  ...
  const saveForm = () => {
    ...
    const payload = { name: form.name.trim(), boundaries, strategy, weight: Number(form.weight) || 0, status: form.status };
    if (editingRow) {
      const res = logisticsService.consolidationRules.update(editingRow.id, payload);
      if (res && res.ok === false) { window.alert(res.message || '保存失败'); return; }
      window.alert('已保存');
    } else {
      const res = logisticsService.consolidationRules.create(payload);
      if (res && res.ok === false) { window.alert(res.message || '新增失败'); return; }
      window.alert('已新增');
    }
  ...
  const handleSubmit = (row) => {
    const res = logisticsService.consolidationRules.submit(row.id);
  ...
  const handleRemove = (row) => {
    ...
    logisticsService.consolidationRules.remove(row.id);
    loadList();
    loadStats();
  };

  const handleExport = () => {
    const res = logisticsService.consolidationRules.list({ keyword: filters.keyword, status: filters.status, approvalStatus: filters.approvalStatus, page: 1, pageSize: 99999 });
    const blob = new Blob([JSON.stringify(res.list, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `集货规则_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const handleImport = () => {
    ...
    arr.forEach((item) => {
      const id = item.id;
      const existing = id ? logisticsService.consolidationRules.get(id) : null;
      if (existing) {
        const res = logisticsService.consolidationRules.update(id, { ...item, id: undefined });
        if (res && res.ok === false) window.alert(res.message);
        else updated++;
      } else {
        const res = logisticsService.consolidationRules.create(item);
        if (res && res.ok === false) window.alert(res.message);
        else created++;
      }
    });
  ...
  const countryDisplay = (row) => row.boundaries?.country ?? row.country ?? '-';

  return (
    <div className="flex flex-col h-full min-h-0">
      <h1 className="text-lg font-semibold text-gray-800 mb-4">集货规则</h1>
            ...
            <tbody>
              {listData.list.map((row) => (
                <tr key={row.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-700">{countryDisplay(row)}</td>
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-[480px] p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-semibold mb-4">{editingRow ? '编辑规则' : '新增规则'}</h2>
            <div className="space-y-3">
              <div><label className="block text-sm text-gray-700 mb-1">名称 *</label><input type="text" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">国家 *</label><input type="text" value={form.country} onChange={(e) => setForm((f) => ({ ...f, country: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" placeholder="如 US" /></div>
              ...
              <div><label className="block text-sm text-gray-700 mb-1">平台（可选）</label><input type="text" value={form.platform} onChange={(e) => setForm((f) => ({ ...f, platform: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">物流商（可选）</label><select value={form.vendorId} onChange={(e) => setForm((f) => ({ ...f, vendorId: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm"><option value="">不限</option>{vendorList.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</select></div>
              <div><label className="block text-sm text-gray-700 mb-1">渠道（可选）</label><select value={form.channelId} onChange={(e) => setForm((f) => ({ ...f, channelId: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm"><option value="">不限</option>{channelList.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              <div><label className="block text-sm text-gray-700 mb-1">含税（可选）</label><select value={form.taxIncluded} onChange={(e) => setForm((f) => ({ ...f, taxIncluded: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm"><option value="">不限</option><option value="true">是</option><option value="false">否</option></select></div>
              <div><label className="block text-sm text-gray-700 mb-1">每批最大单数</label><input type="number" value={form.maxOrdersPerBatch} onChange={(e) => setForm((f) => ({ ...f, maxOrdersPerBatch: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">截单小时 0-23（可选）</label><input type="number" min={0} max={23} value={form.cutoffHourLocal} onChange={(e) => setForm((f) => ({ ...f, cutoffHourLocal: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" placeholder="空不填" /></div>
            </div>
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-[560px] p-6 max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <p className="text-sm text-gray-500 mb-3">粘贴 JSON 数组，同 id 更新，否则新增。国家必填。</p>
...
export default LogisticsConsolidationRuleListPage;
