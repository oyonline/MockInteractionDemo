/**
 * 仓库地址列表页：统计、筛选、表格、ActionBar、新增/编辑/导入/导出、分页。
 */
...
function LogisticsAddressListPage({ onOpenDetail }) {
  ...
  const [form, setForm] = useState({ type: 'warehouse', name: '', receiver: '', phone: '', country: '', state: '', city: '', postcode: '', address: '', status: 'enabled' });
  const [showImportModal, setShowImportModal] = useState(false);
  const [importText, setImportText] = useState('');

  const loadList = useCallback(() => {
    setListData(logisticsService.addresses.list({ keyword: filters.keyword, status: filters.status, approvalStatus: filters.approvalStatus, page, pageSize: PAGE_SIZE }));
  }, [filters.keyword, filters.status, filters.approvalStatus, page]);

  const loadStats = useCallback(() => {
    const all = logisticsService.addresses.list({ page: 1, pageSize: 99999 });
    const enabled = logisticsService.addresses.list({ status: 'enabled', page: 1, pageSize: 1 }).total;
    const pending = logisticsService.addresses.list({ approvalStatus: 'pending', page: 1, pageSize: 1 }).total;
  ...
  const openDetail = (row) => {
    onOpenDetail && onOpenDetail({ id: `addr-${row.id}`, name: `地址-${row.name}`, path: `/logistics/addresses/${row.id}`, data: { id: row.id } });
  };

  const openCreate = () => {
    setEditingRow(null);
    setForm({ type: 'warehouse', name: '', receiver: '', phone: '', country: '', state: '', city: '', postcode: '', address: '', status: 'enabled' });
  ...
  const handleExport = () => {
    const res = logisticsService.addresses.list({ keyword: filters.keyword, status: filters.status, approvalStatus: filters.approvalStatus, page: 1, pageSize: 99999 });
    const blob = new Blob([JSON.stringify(res.list, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `仓库地址_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const handleImport = () => {
    ...
    arr.forEach((item) => {
      const id = item.id;
      const existing = id ? logisticsService.addresses.get(id) : null;
      if (existing) { logisticsService.addresses.update(id, { ...item, id: undefined }); updated++; }
      else { logisticsService.addresses.create(item); created++; }
    });
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <h1 className="text-lg font-semibold text-gray-800 mb-4">仓库地址</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-4 border border-gray-200">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <input type="text" placeholder="名称/收货人/城市/国家/地址" value={filters.keyword} onChange={(e) => setFilters((f) => ({ ...f, keyword: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm w-56 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={openCreate} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">新增地址</button>
          ...
          <table className="w-full text-sm min-w-[900px]">
            <thead className="bg-gray-50 sticky top-0 z-[1]">
              <tr className="border-b">
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600 whitespace-nowrap">城市</th>
                <th className="px-4 py-3 text-left font-medium text-gray-600 whitespace-nowrap">邮编</th>
            ...
            <tbody>
              {listData.list.map((row) => (
                <tr key={row.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{row.postcode || '-'}</td>
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-[480px] p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-semibold mb-4">{editingRow ? '编辑地址' : '新增地址'}</h2>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm text-gray-700 mb-1">类型</label>
                <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm">
                  <option value="warehouse">仓库</option><option value="pickup">提货</option><option value="return">退货</option>
                </select>
              </div>
              ...
              <div><label className="block text-sm text-gray-700 mb-1">收货人</label><input type="text" value={form.receiver} onChange={(e) => setForm((f) => ({ ...f, receiver: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">电话</label><input type="text" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">国家</label><input type="text" value={form.country} onChange={(e) => setForm((f) => ({ ...f, country: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">省/州</label><input type="text" value={form.state} onChange={(e) => setForm((f) => ({ ...f, state: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">城市</label><input type="text" value={form.city} onChange={(e) => setForm((f) => ({ ...f, city: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">邮编</label><input type="text" value={form.postcode} onChange={(e) => setForm((f) => ({ ...f, postcode: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
              <div className="col-span-2"><label className="block text-sm text-gray-700 mb-1">详细地址</label><input type="text" value={form.address} onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" /></div>
...
export default LogisticsAddressListPage;
